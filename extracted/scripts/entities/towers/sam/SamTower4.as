package entities.towers.sam
{
   ...
   public class SamTower4 extends SamTower
   {
      
      public function SamTower4()
      {
         super(2,13);
         this.turret = new Image(Assets.GFX_TOWER_SPRITESHEET,Assets.TURRETS["SAM"][4]);
         this.level = 4;
      ...
      override protected function shoot() : void
      {
         ...
         var m:Array = new Array(new Point(),new Point(),new Point(),new Point(),new Point(),new Point(),new Point(),new Point(),new Point());
         m[0].x = this.centerX - Math.cos(-this.turret.angle * Math.PI / 180) * 8;
         m[0].y = this.centerY - Math.sin(-this.turret.angle * Math.PI / 180) * 8;
         m[1].x = this.centerX - Math.cos(-(this.turret.angle - 40) * Math.PI / 180) * 10;
         m[1].y = this.centerY - Math.sin(-(this.turret.angle - 40) * Math.PI / 180) * 10;
         m[2].x = this.centerX - Math.cos(-(this.turret.angle + 40) * Math.PI / 180) * 10;
         m[2].y = this.centerY - Math.sin(-(this.turret.angle + 40) * Math.PI / 180) * 10;
         m[3].x = this.centerX - Math.cos(-this.turret.angle * Math.PI / 180) * 4;
         m[3].y = this.centerY - Math.sin(-this.turret.angle * Math.PI / 180) * 4;
         m[4].x = this.centerX - Math.cos(-(this.turret.angle - 45) * Math.PI / 180) * 6;
         m[4].y = this.centerY - Math.sin(-(this.turret.angle - 45) * Math.PI / 180) * 6;
         ...
         m[6].x = this.centerX - Math.cos(-(this.turret.angle + 90) * Math.PI / 180) * 4;
         m[6].y = this.centerY - Math.sin(-(this.turret.angle + 90) * Math.PI / 180) * 4;
         m[7].x = this.centerX - Math.cos(-(this.turret.angle - 90) * Math.PI / 180) * 4;
         m[7].y = this.centerY - Math.sin(-(this.turret.angle - 90) * Math.PI / 180) * 4;
         m[8].x = this.centerX;
         m[8].y = this.centerY;
