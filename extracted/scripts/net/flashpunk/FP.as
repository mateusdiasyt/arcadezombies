package net.flashpunk
{
   ...
   public class FP
   {
      ...
      public static function colorLerp(fromColor:uint, toColor:uint, t:Number = 1) : uint
      {
         ...
         var a:uint = uint(fromColor >> 24 & 0xFF);
         var r:uint = uint(fromColor >> 16 & 0xFF);
         var g:uint = uint(fromColor >> 8 & 0xFF);
         var b:uint = uint(fromColor & 0xFF);
         var dA:int = (toColor >> 24 & 0xFF) - a;
         var dR:int = (toColor >> 16 & 0xFF) - r;
         var dG:int = (toColor >> 8 & 0xFF) - g;
         var dB:int = (toColor & 0xFF) - b;
         a += dA * t;
         r += dR * t;
         g += dG * t;
         b += dB * t;
      ...
      {
         if(Boolean(_bitmap[String(source)]))
         {
      ...
      {
         var t:uint = uint(getTimer());
         var e:uint = uint(t - _time);
         _time = t;
         return e;
      }
