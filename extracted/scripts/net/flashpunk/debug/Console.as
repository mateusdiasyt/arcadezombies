package net.flashpunk.debug
{
   ...
   public class Console
   {
      ...
      [Embed(source="../graphics/04B_03__.TTF",embedAsCFF="false",fontFamily="console")]
      private const FONT_SMALL:Class = Console_FONT_SMALL;
      
      [Embed(source="console_logo.png")]
      ...
      public function enable() : void
      {
         ...
         this._fpsRead.graphics.drawRoundRectComplex(0,0,big ? 200 : 100,20,0,0,0,20);
         if(big)
         {
               ...
               if(FP.engine.paused)
               {
                  if(Input.mousePressed)
                  {
                     if(Input.mouseFlashY > 20 && (Input.mouseFlashX > this._debReadText1.width || Input.mouseFlashY < this._debRead.y))
                     {
                        ...
                        {
                           if(Boolean(this.SELECT_LIST.length))
                           {
                              this.startDragging();
                           }
                           else
      ...
      private function renderEntities() : void
      {
         ...
         if(this._debug)
         {
            ...
            for each(e in this.SCREEN_LIST)
            {
               if(this.SELECT_LIST.indexOf(e) < 0)
               {
                  if(Boolean(e.width) && Boolean(e.height))
                  {
      ...
      private function updateLog() : void
      {
         ...
         if(this._paused)
         {
            ...
            if(this.LOG.length > this._logLines)
            {
               ...
               h = FP.clamp(this._logBar.height * (this._logLines / this.LOG.length),12,this._logBar.height - 4);
            ...
            if(Boolean(this.LOG.length))
            {
               i = this.LOG.length > this._logLines ? int(Math.round((this.LOG.length - this._logLines) * this._logScroll)) : 0;
