package net.flashpunk.graphics
{
   ...
   public class Spritemap extends Image
   {
      ...
      override public function updateBuffer(clearBefore:Boolean = false) : void
      {
         ...
         this._rect.y = uint(this._rect.x / this._width) * this._rect.height;
         this._rect.x %= this._width;
         if(_flipped)
         {
      ...
      {
         if(Boolean(this._anim) && !this.complete)
         {
            this._timer += (FP.fixed ? this._anim._frameRate : this._anim._frameRate * FP.elapsed) * this.rate;
            if(this._timer >= 1)
            {
               while(this._timer >= 1)
               {
      ...
      {
         if(Boolean(this._anims[name]))
         {
            throw new Error("Cannot have multiple animations with the same name");
         }
         (this._anims[name] = new Anim(name,frames,frameRate,loop))._parent = this;
      ...
      {
         return Boolean(this._anim) ? this._index : 0;
      }
